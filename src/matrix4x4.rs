...
impl Matrix4x4 {
    ...
    pub fn inverse(self) -> Matrix4x4 {
        if cfg!(target_arch = "aarch64") {
            ...
            self.inverse_no_simd()
        }
    }

    #[inline(always)]
    fn inverse_no_simd(self: Matrix4x4) -> Matrix4x4 {
    ...
    fn inverse_neon(self: Matrix4x4) -> Matrix4x4 {
        let c0 = self.c0; let c1 = self.c1;
        let c2 = self.c2; let c3 = self.c3;
        ...
        let coef22 = c1[0] * c3[1] - c3[0] * c1[1];
        let coef23 = c1[0] * c2[1] - c2[0] * c1[1];
        ...
        let vec0 = Vector4::new(c1[0], c0[0], c0[0], c0[0]);
        let vec1 = Vector4::new(c1[1], c0[1], c0[1], c0[1]);
    ...
    fn calc_ax_mul_ay_sub_b_mul_c(ax: Vector4, ay: Vector4, b: Vector4, c: Vector4) -> Vector4 {
        unsafe {
            ...
            let acc_reg = vmulq_f32(l_acc, r_acc);

            let b_reg = vld1q_f32(&b as *const Vector4 as *const f32);
            let c_reg = vld1q_f32(&c as *const Vector4 as *const f32);

            let res_get = vmlsq_f32(acc_reg, b_reg, c_reg);

            let mut result: MaybeUninit<Vector4> = MaybeUninit::uninit();
            vst1q_f32(result.as_mut_ptr() as *mut f32, res_get);
            result.assume_init()
        }
    }
}
