...
// Generate useful functions for Vector type except from Vector4 because it's using SIMD
...
fn create_vec_math() -> String {
    ...
    for dim in vec_dims {
        ...
        result.write_fmt(format_args!("impl {vec_type} {{\n\n")).unwrap();
...
fn create_expanded_math_func(func_name: &str, vec_dimension: usize, additional_args: &[FuncArg]) -> String {
    ...
    let func_call_args = VEC_FIELDS.iter()
        .take(vec_dimension)
        .map(|f| {
            format!("{}({})",
                    func_name,
...
fn create_all_func(dim: usize) -> String {
    format!(
        ...
        {{ Vector{dim}::new({}) }}\n", (0..dim).map(|_| { "value" }).join(",")
    )
}

fn create_zero_func(dim: usize) -> String {
    format!(
        "#[inline] pub fn zero() -> Vector{dim} {{ Vector{dim}::all(0f32) }}\n"
    )
}

fn create_one_func(dim: usize) -> String {
    format!(
        "#[inline] pub fn one() -> Vector{dim} {{ Vector{dim}::all(1f32) }}\n"
    )
}

fn create_length_funcs(dim: usize) -> String {
    let formula = VEC_FIELDS.iter()
        .take(dim)
...
fn create_normalized_func(dim: usize) -> String {
    format!("
        #[inline] pub fn normalized(self) -> Vector{dim} {{ self / self.length() }}
    ")
}
...
fn create_component_wise_vec_func(
    operator_trait: &str, operator_token: &str, dim: usize
) -> String {
    let simd_prefix = get_simd_prefix(dim);
    let func_name = operator_trait.to_lowercase();
    let new_vec_result_args =  VEC_FIELDS.iter()
        .take(dim)
